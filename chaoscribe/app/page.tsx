...
const likedMap = new Map<string, Article>(); 
            ...
            <div className="flex min-h-screen flex-col justify-center items-center px-6 py-12 lg:px-8">
                {articles.map((article: Article, i: number) => {
                    if (article.chaosLevel == chaosLevel && (article.title.toLowerCase().includes(search.toLowerCase()) || 
                        ...
                        return (
                            <div key={i} className="flex flex-col justify-between w-7/12 h-100 p-4 mb-4 bg-slate-950 border rounded-lg shadow-md dark:border-gray-700 my-7">
                                ...
                                <div className="items-end flex items-center justify-around mt-4"> 
                                    <div>
                                        <button className="flex items-center px-3 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-300 dark:focus:ring-gray-600" onClick={() => { if (liked.filter(l => l.id == article.id).length < 1) setLiked([...liked, article]); console.log(liked) }}>
                                            ...
                                            <span className="ml-2">{`${liked.filter(l => l.id == article.id).length} ${liked.filter(l => l.id == article.id).length === 1 ? `Like` : 'Likes'}`}</span>
                                        </button>
                                    </div>
                                    <div>
